    ...
    if (accelerometers.size() > 0) {
        if (t < accelerometers.front().t) {
            ...
            while (accelerometers.size() > 0 && t >= accelerometers.front().t) {
                ...
                vector<3> w = gyroscopes[0].w + lambda * (vector<3>{x, y, z} - gyroscopes[0].w);            // linear interpolation at acc.t
...
// aligned acc and gyro at same time with track_gyroscope and track_accelerometer
